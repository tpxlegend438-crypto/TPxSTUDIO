...
<span class="syn-kw">public class</span> <span class="syn-type">TPXPlugin</span> {
  ...
  cpp: {
    ...
    code: `<span class="syn-kw">#include</span> <span class="syn-str">&lt;iostream&gt;</span>
<span class="syn-kw">#include</span> <span class="syn-str">&lt;tpx/network.hpp&gt;</span>

<span class="syn-cm">// Native C++ Multi-Threaded Packet Buffer</span>
<span class="syn-type">int</span> <span class="syn-fn">main</span>() {
    tpx::SocketEngine engine(<span class="syn-str">"0.0.0.0"</span>, <span class="syn-num">25565</span>);
    engine.set_thread_concurrency(<span class="syn-num">16</span>);
    engine.enable_hardware_acceleration(<span class="syn-kw">true</span>);

    engine.on_packet([](<span class="syn-kw">const</span> tpx::Packet&amp; p) {
        p.forward_optimized();
    });

    std::cout &lt;&lt; <span class="syn-str">"[TPX C++] High-FPS UDP Router active."</span> &lt;&lt; std::endl;
    engine.listen();
    <span class="syn-kw">return</span> <span class="syn-num">0</span>;
}`
  },
  kotlin: {
    file: "TPXAndroidService.kt",
    code: `<span class="syn-kw">package</span> com.tpxstudio.mobile

<span class="syn-kw">import</span> kotlinx.coroutines.*
<span class="syn-kw">import</span> com.tpxstudio.sdk.TPXClient

<span class="syn-cm">// Kotlin Coroutines Mobile Realtime Sync</span>
<span class="syn-kw">class</span> <span class="syn-type">TPXSyncEngine</span> {
    <span class="syn-kw">private val</span> client = <span class="syn-type">TPXClient</span>(<span class="syn-str">"tpx_mobile_token"</span>)

    <span class="syn-kw">fun</span> <span class="syn-fn">startRealtimeStream</span>() = CoroutineScope(Dispatchers.IO).<span class="syn-fn">launch</span> {
        client.events.<span class="syn-fn">subscribe</span>(<span class="syn-str">"feed_updates"</span>).<span class="syn-fn">collect</span> { event -&gt;
            println(<span class="syn-str">"[TPX Kotlin] Event Received: \${event.payload}"</span>)
        }
    }
}`
  },
  swift: {
    file: "TPXAppleEngine.swift",
    code: `<span class="syn-kw">import</span> Foundation
<span class="syn-kw">import</span> TPXCore

<span class="syn-cm">// Swift Concurrency for iOS & macOS Engines</span>
<span class="syn-kw">actor</span> <span class="syn-type">TPXMediaManager</span> {
    <span class="syn-kw">let</span> client = <span class="syn-type">TPXClient</span>(apiKey: <span class="syn-str">"tpx_apple_live"</span>)

    <span class="syn-kw">func</span> <span class="syn-fn">preloadStreams</span>() <span class="syn-kw">async throws</span> {
        <span class="syn-kw">let</span> channels = <span class="syn-kw">try await</span> client.channels.<span class="syn-fn">fetchLive</span>()
        print(<span class="syn-str">"[TPX Swift] Connected \(channels.count) live HLS feeds."</span>)
    }
}`
  },
  dart: {
...
<span class="syn-cm">// Cross-Platform Flutter Media Engine</span>
...
<span class="syn-kw">class</span> <span class="syn-type">Program</span> {
  ...
  sql: {
    file: "schema.sql",
    code: `<span class="syn-cm">-- High Performance PostgreSQL Multi-Tenant DB</span>
<span class="syn-kw">CREATE TABLE</span> <span class="syn-type">tpx_clusters</span> (
    <span class="syn-prop">id</span> <span class="syn-type">UUID PRIMARY KEY DEFAULT</span> gen_random_uuid(),
    <span class="syn-prop">client_id</span> <span class="syn-type">VARCHAR(64) NOT NULL</span>,
    <span class="syn-prop">region</span> <span class="syn-type">VARCHAR(32) DEFAULT</span> <span class="syn-str">'in-mumbai-01'</span>,
    <span class="syn-prop">latency_ms</span> <span class="syn-type">DECIMAL(4,2) DEFAULT</span> <span class="syn-num">11.45</span>,
    <span class="syn-prop">status</span> <span class="syn-type">VARCHAR(16) DEFAULT</span> <span class="syn-str">'online'</span>,
    <span class="syn-prop">created_at</span> <span class="syn-type">TIMESTAMPTZ DEFAULT NOW</span>()
);

<span class="syn-kw">CREATE INDEX</span> <span class="syn-fn">idx_tpx_latency</span> <span class="syn-kw">ON</span> <span class="syn-type">tpx_clusters</span> (latency_ms);`
  },
  bash: {
    file: "deploy_node.sh",
    code: `<span class="syn-cm">#!/usr/bin/env bash</span>
<span class="syn-kw">set</span> -euo pipefail

<span class="syn-cm"># TPX Studio Cloud Node Auto-Provisioning</span>
echo <span class="syn-str">"==&gt; [TPX DevOps] Initializing Docker Container Cluster..."</span>
docker compose -f docker-compose.prod.yml up -d --build --scale node_worker=<span class="syn-num">4</span>

echo <span class="syn-str">"==&gt; [TPX DevOps] Applying Cloudflare Edge SSL Firewall rules..."</span>
curl -sX POST <span class="syn-str">"https://api.cloudflare.com/client/v4/zones/\${CF_ZONE}/purge_cache"</span> \\
     -H <span class="syn-str">"Authorization: Bearer \${CF_TOKEN}"</span> \\
     -d <span class="syn-str">'{"purge_everything":true}'</span>

echo <span class="syn-str">"==&gt; [TPX DevOps] Deployment Complete. Zero downtime achieved."</span>`
  }
};
...
function switchCodeLang(lang) {
  ...
  if (output && filename && codeSnippets[lang]) {
...
const webTabContent = {
  landing: `
    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-6 items-center">
      <div>
        ...
        <h3 class="font-display text-lg sm:text-2xl font-bold text-white mb-2">High-Conversion Landing Pages</h3>
        <p class="mb-3.5 text-gray-300 text-xs sm:text-sm leading-relaxed">Precision-crafted single-page architectures built on React & Next.js. Engineered with conversion psychology, smooth micro-interactions, responsive typography, and sub-second load times.</p>
        ...
        <a href="https://wa.me/917047336254?text=Hi%20TPX%20STUDIO,%20I%20want%20to%20order%20a%20High-Conversion%20Landing%20Page." target="_blank" class="inline-flex items-center gap-1.5 font-display text-xs bg-gold-primary text-black px-4 sm:px-5 py-2.5 rounded-full font-bold hover:glow-gold transition-all">ORDER LANDING PAGE <span class="material-symbols-outlined text-sm">arrow_forward</span></a>
      </div>
      <div class="glass-card p-4 sm:p-5 rounded-2xl border-gold-primary/30 text-xs font-mono">
  ...
  multipage: `
    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-6 items-center">
      <div>
        ...
        <h3 class="font-display text-lg sm:text-2xl font-bold text-white mb-2">Full-Stack Web Applications</h3>
        <p class="mb-3.5 text-gray-300 text-xs sm:text-sm leading-relaxed">Enterprise-grade multi-page web applications with complete routing, server-side rendering, user authentication, role-based permissions, and custom API backends.</p>
        <ul class="space-y-1.5 font-mono text-xs text-text-dim mb-4">
        ...
        <a href="https://wa.me/917047336254?text=Hi%20TPX%20STUDIO,%20I%20want%20a%20Full-Stack%20Web%20App." target="_blank" class="inline-flex items-center gap-1.5 font-display text-xs bg-gold-metallic text-black px-4 sm:px-5 py-2.5 rounded-full font-bold hover:glow-amber transition-all">BUILD WEB APP <span class="material-symbols-outlined text-sm">arrow_forward</span></a>
      </div>
      <div class="glass-card p-4 sm:p-5 rounded-2xl border-gold-metallic/30 text-xs font-mono">
  ...
  ecommerce: `
    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-6 items-center">
      <div>
        ...
        <h3 class="font-display text-lg sm:text-2xl font-bold text-white mb-2">E-Commerce & Digital Storefronts</h3>
        <p class="mb-3.5 text-gray-300 text-xs sm:text-sm leading-relaxed">High-converting online webstores with frictionless checkout flows, Razorpay/Stripe automated gateways, inventory synchronization, and instant automated digital delivery.</p>
        <ul class="space-y-1.5 font-mono text-xs text-text-dim mb-4">
        ...
        <a href="https://wa.me/917047336254?text=Hi%20TPX%20STUDIO,%20I%20want%20an%20E-Commerce%20Store." target="_blank" class="inline-flex items-center gap-1.5 font-display text-xs bg-gold-light text-black px-4 sm:px-5 py-2.5 rounded-full font-bold transition-all">LAUNCH STOREFRONT <span class="material-symbols-outlined text-sm">arrow_forward</span></a>
      </div>
      <div class="glass-card p-4 sm:p-5 rounded-2xl border-gold-light/30 text-xs font-mono">
  ...
  saas: `
    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-6 items-center">
      <div>
        ...
        <h3 class="font-display text-lg sm:text-2xl font-bold text-white mb-2">SaaS Dashboards & Admin Portals</h3>
        <p class="mb-3.5 text-gray-300 text-xs sm:text-sm leading-relaxed">Interactive analytical dashboards featuring real-time data streaming, dynamic chart visualizations, filter pipelines, and customizable control panels.</p>
        <ul class="space-y-1.5 font-mono text-xs text-text-dim mb-4">
        ...
        <a href="https://wa.me/917047336254?text=Hi%20TPX%20STUDIO,%20I%20want%20a%20SaaS%20Dashboard." target="_blank" class="inline-flex items-center gap-1.5 font-display text-xs bg-gold-amber text-black px-4 sm:px-5 py-2.5 rounded-full font-bold transition-all">BUILD SAAS DASHBOARD <span class="material-symbols-outlined text-sm">arrow_forward</span></a>
      </div>
      <div class="glass-card p-4 sm:p-5 rounded-2xl border-gold-amber/30 text-xs font-mono">
  ...
  designsystem: `
    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-6 items-center">
      <div>
        ...
        <h3 class="font-display text-lg sm:text-2xl font-bold text-white mb-2">Design Systems & Brand UI</h3>
        <p class="mb-3.5 text-gray-300 text-xs sm:text-sm leading-relaxed">Comprehensive component token libraries, typography scales, accessibility guidelines, and dark luxury interface kits built for cohesive brand identity across all platforms.</p>
        <ul class="space-y-1.5 font-mono text-xs text-text-dim mb-4">
        ...
        <a href="https://wa.me/917047336254?text=Hi%20TPX%20STUDIO,%20I%20want%20a%20Design%20System." target="_blank" class="inline-flex items-center gap-1.5 font-display text-xs bg-gold-primary text-black px-4 sm:px-5 py-2.5 rounded-full font-bold hover:glow-gold transition-all">GET DESIGN SYSTEM <span class="material-symbols-outlined text-sm">arrow_forward</span></a>
      </div>
      <div class="glass-card p-4 sm:p-5 rounded-2xl border-gold-primary/30 text-xs font-mono">
  ...
  mobile: `
    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-6 items-center">
      <div>
        ...
        <h3 class="font-display text-lg sm:text-2xl font-bold text-white mb-2">Mobile & Responsive Interfaces</h3>
        <p class="mb-3.5 text-gray-300 text-xs sm:text-sm leading-relaxed">Fluid layouts engineered for smartphones, tablets, laptops, and ultra-wide displays. Built with progressive web app (PWA) offline support and native-feeling touch gestures.</p>
        <ul class="space-y-1.5 font-mono text-xs text-text-dim mb-4">
        ...
        <a href="https://wa.me/917047336254?text=Hi%20TPX%20STUDIO,%20I%20want%20Mobile%20UI%20Architecture." target="_blank" class="inline-flex items-center gap-1.5 font-display text-xs bg-gold-metallic text-black px-4 sm:px-5 py-2.5 rounded-full font-bold hover:glow-amber transition-all">ORDER RESPONSIVE UI <span class="material-symbols-outlined text-sm">arrow_forward</span></a>
      </div>
      <div class="glass-card p-4 sm:p-5 rounded-2xl border-gold-metallic/30 text-xs font-mono">
  ...
  analytics: `
    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-6 items-center">
      <div>
        <div class="inline-block px-2.5 py-0.5 rounded-full bg-cyan-400/10 border border-cyan-400/30 text-cyan-300 font-mono text-[10px] mb-2 font-bold">REALTIME TELEMETRY</div>
        <h3 class="font-display text-lg sm:text-2xl font-bold text-white mb-2">Live Analytics & Monitoring UI</h3>
        <p class="mb-3.5 text-gray-300 text-xs sm:text-sm leading-relaxed">High-frequency telemetry dashboards with real-time throughput metrics, server heatmaps, node latency tracking, and custom alert webhooks.</p>
        <ul class="space-y-1.5 font-mono text-xs text-text-dim mb-4">
          <li class="flex items-center gap-2 text-cyan-200"><span class="material-symbols-outlined text-sm text-cyan-400">check_circle</span> Live WebSockets Stream Pipeline</li>
          <li class="flex items-center gap-2 text-cyan-200"><span class="material-symbols-outlined text-sm text-cyan-400">check_circle</span> Sub-Millisecond Time-Series Visuals</li>
        </ul>
        <a href="https://wa.me/917047336254?text=Hi%20TPX%20STUDIO,%20I%20want%20Analytics%20Dashboard%20UI." target="_blank" class="inline-flex items-center gap-1.5 font-display text-xs bg-cyan-400 text-black px-4 sm:px-5 py-2.5 rounded-full font-bold hover:shadow-[0_0_20px_rgba(6,182,212,0.4)] transition-all">GET ANALYTICS UI <span class="material-symbols-outlined text-sm">arrow_forward</span></a>
      </div>
      <div class="glass-card p-4 sm:p-5 rounded-2xl border-cyan-400/30 text-xs font-mono">
        <div class="text-cyan-400 font-bold mb-2">// Telemetry Channel</div>
        <div class="space-y-2 text-gray-300">
          <div class="flex justify-between border-b border-white/10 pb-1"><span>Packet Ingestion Rate</span><span class="text-green-400">25,000 / sec</span></div>
          <div class="flex justify-between border-b border-white/10 pb-1"><span>Render FPS</span><span class="text-green-400">60.0 FPS</span></div>
          <div class="flex justify-between"><span>Alert Trigger Latency</span><span class="text-cyan-300">&lt; 15ms</span></div>
        </div>
      </div>
    </div>`,
  web3: `
    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-6 items-center">
      <div>
        <div class="inline-block px-2.5 py-0.5 rounded-full bg-purple-400/10 border border-purple-400/30 text-purple-300 font-mono text-[10px] mb-2 font-bold">DECENTRALIZED UI</div>
        <h3 class="font-display text-lg sm:text-2xl font-bold text-white mb-2">Web3 & Token Gateways</h3>
        <p class="mb-3.5 text-gray-300 text-xs sm:text-sm leading-relaxed">Modern Web3 decentralized dashboards with wallet connectivity (MetaMask, Phantom, WalletConnect), smart contract read/write UI, and token-gated server rewards.</p>
        <ul class="space-y-1.5 font-mono text-xs text-text-dim mb-4">
          <li class="flex items-center gap-2 text-purple-200"><span class="material-symbols-outlined text-sm text-purple-400">check_circle</span> Multi-Chain Wallet Connect Integrations</li>
          <li class="flex items-center gap-2 text-purple-200"><span class="material-symbols-outlined text-sm text-purple-400">check_circle</span> Gas-Optimized Contract Interactions</li>
        </ul>
        <a href="https://wa.me/917047336254?text=Hi%20TPX%20STUDIO,%20I%20want%20Web3%20UI%20Architecture." target="_blank" class="inline-flex items-center gap-1.5 font-display text-xs bg-purple-400 text-black px-4 sm:px-5 py-2.5 rounded-full font-bold hover:shadow-[0_0_20px_rgba(168,85,247,0.4)] transition-all">GET WEB3 UI <span class="material-symbols-outlined text-sm">arrow_forward</span></a>
      </div>
      <div class="glass-card p-4 sm:p-5 rounded-2xl border-purple-400/30 text-xs font-mono">
        <div class="text-purple-400 font-bold mb-2">// Web3 Contract Connector</div>
        <div class="space-y-2 text-gray-300">
          <div class="flex justify-between border-b border-white/10 pb-1"><span>Supported Chains</span><span class="text-purple-300">EVM & Solana</span></div>
          <div class="flex justify-between border-b border-white/10 pb-1"><span>Signer Security</span><span class="text-green-400">EIP-712 Typed</span></div>
          <div class="flex justify-between"><span>RPC Edge Endpoint</span><span class="text-purple-300">Global Fallback</span></div>
        </div>
      </div>
    </div>`
};
...
// ==========================================
// 4. INTERACTIVE SERVICE DETAIL MODAL (PHOTOS & SPECS)
// ==========================================
const serviceModalData = {
  web: {
    badge: "CUSTOM WEB ARCHITECTURE",
    title: "Full-Stack Web Architecture & SaaS Portals",
    desc: "From conversion-optimized landing pages to multi-tenant SaaS dashboards, we develop responsive, secure, and blazing-fast web engines. Built with Next.js, React, TypeScript, and Tailwind CSS with 100/100 Lighthouse performance metrics.",
    photos: [
    ...
    features: [
      "Sub-second page loads with server-side rendering (SSR)",
      "Automated Discord OAuth2, login systems, and role synchronization",
      "Integrated checkout with Razorpay, Stripe, and crypto gateways",
      "Complete responsive design tested across all mobile, tablet, and 4K viewports",
      "100% full source code ownership with GitHub repository handover"
    ],
    ctaText: "ORDER CUSTOM WEB ARCHITECTURE",
    ctaLink: "https://wa.me/917047336254?text=Hi%20TPX%20STUDIO,%20I%20want%20to%20order%20Custom%20Web%20Architecture."
  },
  ai: {
    badge: "AI DEVELOPMENT & AUTOMATION",
    title: "Open-Source AI & Autonomous Workflows",
    desc: "Empower your business with cutting-edge artificial intelligence. We engineer fine-tuned LLM conversational assistants, LangChain vector retrieval systems, autonomous scraping bots, and intelligent workflow automation.",
    photos: [
    ...
    features: [
      "Custom vector databases (Pinecone, Qdrant, ChromaDB)",
      "24/7 intelligent customer support bots for Discord, Telegram & WhatsApp",
      "Context-aware RAG pipelines fine-tuned on your internal documents",
      "Automated content generation, summarization, and translation microservices",
      "Zero latency streaming responses with OpenAI, Anthropic, and open-source models"
    ],
    ctaText: "INTEGRATE AI WORKFLOW",
    ctaLink: "https://wa.me/917047336254?text=Hi%20TPX%20STUDIO,%20I%20want%20to%20integrate%20AI%20Workflows."
  },
  baas: {
    badge: "BACKEND AS A SERVICE",
    title: "Self-Hostable Backend as a Service (BaaS)",
    desc: "Ship software in days instead of months. Our white-label, self-hostable BaaS engine gives you drag-and-drop database schema creation, instant REST/GraphQL endpoints, JWT role guards, and zero recurring cloud bills.",
    photos: [
    ...
    features: [
      "Visual drag-and-drop PostgreSQL & MongoDB schema modeling",
      "Instant autogenerated secure REST & GraphQL endpoints",
      "Built-in JWT auth, password hashing, and user permission groups",
      "Ready to self-host on Docker, Railway, Render, or bare-metal VPS ($0 cloud fee)",
      "Real-time WebSocket subscriptions for live data synchronization"
    ],
    ctaText: "DEPLOY BAAS ENGINE",
    ctaLink: "https://wa.me/917047336254?text=Hi%20TPX%20STUDIO,%20I%20want%20to%20deploy%20a%20BaaS%20Platform."
  },
  voxel: {
    badge: "3D VOXEL ASSETS",
    title: "3D Voxel Models & Sandbox Game Props",
    desc: "Specialized 3D modeling and animation designed for Minecraft servers, Hytalelike sandboxes, and custom game engines. Every asset is optimized with clean geometry and low polycounts to ensure rock-solid 60+ FPS.",
    photos: [
      "https://images.unsplash.com/photo-1614680376593-902f749f7ba3?auto=format&fit=crop&w=800&q=80",
      "https://images.unsplash.com/photo-1563089145-599997674d42?auto=format&fit=crop&w=800&q=80",
      "https://images.unsplash.com/photo-1550745165-9bc0b252726f?auto=format&fit=crop&w=800&q=80"
    ],
    features: [
      "40+ ready-to-use Blockbench JSON and OBJ models",
      "Custom weapon, armor, vehicle, and interactive creature rigs",
      "Pre-configured Java plugin configs (ItemsAdder, Oraxen, MythicMobs)",
      "Custom Pterodactyl UI theme packs and custom server icons",
      "High FPS polycount optimization tested under heavy multiplayer loads"
    ],
    ctaText: "GET 3D VOXEL PACK",
    ctaLink: "https://wa.me/917047336254?text=Hi%20TPX%20STUDIO,%20I%20want%20to%20order%203D%20Voxel%20Assets."
  },
  servers: {
    badge: "GAME SERVERS & NODES",
    title: "Low-Latency Game Server Hub (<12ms)",
    desc: "Enterprise-grade game server infrastructure hosted in high-tier Indian datacenters. Equipped with multi-layer DDoS mitigation shields, high-frequency multi-core CPUs, NVMe SSD storage, and automated Pterodactyl panel deployments.",
    photos: [
    ...
    features: [
      "Ultra-low latency <12ms average ping across India and Southeast Asia",
      "Automatic Layer 4 & Layer 7 DDoS shield with real-time attack mitigation",
      "Pterodactyl Game Panel with full console root control and SFTP access",
      "Automated daily offsite snapshot backups with one-click restore",
      "BungeeCord and Velocity proxy routing setup for multi-server networks"
    ],
    ctaText: "DEPLOY GAME NODE",
    ctaLink: "https://wa.me/917047336254?text=Hi%20TPX%20STUDIO,%20I%20want%20to%20deploy%20a%20Game%20Server%20Node."
  },
  media: {
    badge: "VIRAL MEDIA PRODUCTION",
    title: "Viral Shorts & Cinema Video Edits",
    desc: "High-impact video production engineered specifically for high audience retention on Instagram Reels, YouTube Shorts, and TikTok. Includes kinetic typography, custom sound design (SFX), and gaming storytelling cinema.",
    photos: [
    ...
    features: [
      "Engaging kinetic captions styled to maximize viewer watch time",
      "Custom cinematic SFX sound mastering and mood-matched audio layering",
      "Dynamic zooms, motion blur, and glitch micro-transitions",
      "Optimized 9:16 vertical shorts and 16:9 4K YouTube long-form edits",
      "Fast 24-48 hour turnaround with 2 free revision rounds"
    ],
    ctaText: "ORDER VIDEO EDIT",
    ctaLink: "https://wa.me/917047336254?text=Hi%20TPX%20STUDIO,%20I%20want%20to%20order%20Viral%20Video%20Editing."
  }
};
...
function openServiceModal(serviceKey) {
  ...
  const photoCards = data.photos.map((src, i) => `
    <div class="h-28 sm:h-40 rounded-2xl overflow-hidden border border-white/10 bg-black/40">
      <img src="${src}" alt="${data.title} photo ${i+1}" class="w-full h-full object-cover hover:scale-110 transition-transform duration-500"/>
    </div>
  `).join('');

  const featureItems = data.features.map(f => `
    <li class="flex items-start gap-2 text-xs sm:text-sm text-gray-200">
      <span class="material-symbols-outlined text-gold-primary text-sm shrink-0 mt-0.5">check_circle</span>
      <span>${f}</span>
    </li>
  `).join('');

  container.innerHTML = `
    <span class="inline-block px-3 py-0.5 rounded-full bg-gold-primary/20 text-gold-primary font-mono text-[10px] sm:text-xs font-bold mb-2.5 sm:mb-3">${data.badge}</span>
    <h3 class="font-display text-xl sm:text-3xl font-extrabold text-white mb-2 sm:mb-3">${data.title}</h3>
    <p class="text-xs sm:text-sm text-gray-300 leading-relaxed mb-4 sm:mb-6">${data.desc}</p>
    
    <div class="mb-4 sm:mb-6">
      <div class="font-mono text-xs text-gold-light font-bold mb-2.5 flex items-center gap-1">
        <span class="material-symbols-outlined text-sm">photo_library</span> PRODUCTION PHOTO GALLERY
      </div>
    ...
    <div class="mb-5 sm:mb-6 bg-obsidian-dark/90 p-3.5 sm:p-5 rounded-2xl border border-white/10">
      <div class="font-mono text-[11px] sm:text-xs text-gold-primary font-bold mb-2.5">// TECHNICAL SPECIFICATIONS &amp; DELIVERABLES</div>
    ...
    <div class="flex flex-col sm:flex-row items-center gap-2.5 sm:gap-3">
      <a href="${data.ctaLink}" target="_blank" class="w-full sm:w-auto font-display text-xs bg-gradient-to-r from-gold-primary via-gold-metallic to-gold-amber text-obsidian-black px-6 py-3 rounded-full font-extrabold hover:glow-gold transition-all text-center flex items-center justify-center gap-2 shadow-xl">
        <span class="material-symbols-outlined text-sm sm:text-base">rocket_launch</span> ${data.ctaText}
      </a>
      <button onclick="closeServiceModal()" class="w-full sm:w-auto font-display text-xs bg-white/10 text-white px-5 py-3 rounded-full font-bold hover:bg-white/20 transition-all text-center">
        CLOSE WINDOW
      </button>
    </div>
  `;
...
// ==========================================
// 5. CLIENT STORIES CAROUSEL (< > NAVIGATION)
// ==========================================
...
function nextStory() {
  const totalStories = getStoryCards().length;
  if (totalStories === 0) return;
...
// ==========================================
// 7. PROJECT BUILDER ONBOARDING (WHATSAPP DISPATCH)
// ==========================================
function launchProjectBuilder() {
  const service = document.getElementById('builder-service')?.value || 'Web Architecture';
  const budget = document.getElementById('builder-budget')?.value || '₹5,000+';
  const nameInput = document.getElementById('builder-name')?.value.trim() || 'Client';
  const timeline = document.getElementById('builder-timeline')?.value || 'Standard';

  const msg = encodeURIComponent(`Hi TPX STUDIO,\n\nI am ${nameInput} and would like to initiate a new project protocol:\n\n• Service: ${service}\n• Budget: ${budget}\n• Timeline: ${timeline}\n\nPlease share the proposal and next steps!`);
  window.open(`https://wa.me/917047336254?text=${msg}`, '_blank');
}

// ==========================================
// 8. MULTILINGUAL TRANSLATION (i18n) ENGINE
// ==========================================
const i18nData = {
  en: {
    nav_build: "BUILD",
    ...
    showcase_desc: "Explore production work across custom Web Apps, Game Datacenters, 3D Voxel Engine assets, AI Workspaces, and Media Cinema.",
  ...
  de: {
    ...
    story4_text: `"Das BaaS-Setup spart uns monatlich ta