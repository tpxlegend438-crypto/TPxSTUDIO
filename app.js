...
<span class="syn-kw">#include</span> <span class="syn-str">&lt;tpx/matrix_engine.hpp&gt;</span>

<span class="syn-cm">// High-Performance C++ Simulation & Matrix Renderer</span>
<span class="syn-type">int</span> <span class="syn-fn">main</span>() {
    tpx::MatrixEngine engine;
    engine.<span class="syn-fn">InitializeThreads</span>(<span class="syn-num">16</span>);
    
    <span class="syn-kw">auto</span> status = engine.<span class="syn-fn">AllocateNodes</span>(<span class="syn-str">"in-mumbai-cluster"</span>);
    std::cout &lt;&lt; <span class="syn-str">"[TPX C++] Engine Boot: "</span> &lt;&lt; status.latency_ms &lt;&lt; <span class="syn-str">"ms\\n"</span>;
    <span class="syn-kw">return</span> <span class="syn-num">0</span>;
}`
  },
  kotlin: {
    file: "TPXAndroidClient.kt",
    code: `<span class="syn-kw">package</span> com.tpx.studio.mobile

<span class="syn-kw">import</span> kotlinx.coroutines.flow.*

<span class="syn-cm">// Native Mobile Jetpack Compose Streaming Flow</span>
<span class="syn-kw">class</span> <span class="syn-type">TPXAndroidClient</span>(<span class="syn-kw">private val</span> apiKey: <span class="syn-type">String</span>) {
    <span class="syn-kw">fun</span> <span class="syn-fn">streamMediaFeed</span>(channelId: <span class="syn-type">String</span>): <span class="syn-type">Flow</span>&lt;<span class="syn-type">MediaFrame</span>&gt; = flow {
        emit(<span class="syn-type">MediaFrame</span>(status = <span class="syn-str">"BUFFERING"</span>, fps = <span class="syn-num">60</span>))
    }
}`
  },
  swift: {
    file: "TPXiOSClient.swift",
    code: `<span class="syn-kw">import</span> SwiftUI
<span class="syn-kw">import</span> TPXCore

<span class="syn-cm">// iOS Swift / SwiftUI Native Interface Engine</span>
<span class="syn-kw">struct</span> <span class="syn-type">TPXStreamView</span>: <span class="syn-type">View</span> {
    <span class="syn-kw">@StateObject private var</span> client = <span class="syn-type">TPXClient</span>(apiKey: <span class="syn-str">"tpx_apple_live"</span>)

    <span class="syn-kw">var</span> body: <span class="syn-kw">some</span> <span class="syn-type">View</span> {
        <span class="syn-type">Text</span>(<span class="syn-str">"TPX Studio Node Active: \\(client.activePing)ms"</span>)
            .<span class="syn-fn">foregroundColor</span>(.<span class="syn-prop">yellow</span>)
    }
}`
  },
  dart: {
...
<span class="syn-kw">class</span> <span class="syn-type">Program</span> {
  ...
  sql: {
    file: "schema.sql",
    code: `<span class="syn-cm">-- High-Concurrency PostgreSQL Database Architecture</span>
<span class="syn-kw">CREATE TABLE</span> tpx_servers (
    id <span class="syn-type">UUID PRIMARY KEY DEFAULT</span> gen_random_uuid(),
    node_name <span class="syn-type">VARCHAR</span>(<span class="syn-num">100</span>) <span class="syn-kw">NOT NULL</span>,
    region <span class="syn-type">VARCHAR</span>(<span class="syn-num">50</span>) <span class="syn-kw">DEFAULT</span> <span class="syn-str">'in-mumbai'</span>,
    ddos_shield_active <span class="syn-type">BOOLEAN DEFAULT TRUE</span>,
    latency_ms <span class="syn-type">NUMERIC</span>(<span class="syn-num">5</span>, <span class="syn-num">2</span>) <span class="syn-kw">DEFAULT</span> <span class="syn-num">11.40</span>,
    created_at <span class="syn-type">TIMESTAMP WITH TIME ZONE DEFAULT NOW</span>()
);`
  },
  bash: {
    file: "deploy_node.sh",
...
<span class="syn-cm"># Auto-Provision High-FPS India Datacenter Cluster</span>
<span class="syn-fn">echo</span> <span class="syn-str">"[*] Initializing TPX Node Shield on $(uname -m)..."</span>
docker compose -f docker-compose.prod.yml up -d --scale matrix_worker=<span class="syn-num">8</span>
<span class="syn-fn">echo</span> <span class="syn-str">"[+] Cluster Online. Ping: &lt;12ms across APAC."</span>`
  }
};
...
const webTabContent = {
  ...
  analytics: `
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
      <div>
        <div class="inline-block px-2.5 py-0.5 rounded-full bg-cyan-400/10 border border-cyan-400/30 text-cyan-400 font-mono text-[10px] mb-2 font-bold">REALTIME TELEMETRY</div>
        <h3 class="font-display text-xl sm:text-2xl font-bold text-white mb-2">Real-Time Analytics & Data Views</h3>
        <p class="mb-4 text-gray-300 text-xs sm:text-sm leading-relaxed">High-density telemetry dashboards with WebSocket streaming, query filters, multi-currency conversion charts, and exportable server log graphs.</p>
        <ul class="space-y-1.5 font-mono text-xs text-text-dim mb-4">
          <li class="flex items-center gap-2 text-cyan-200"><span class="material-symbols-outlined text-sm text-cyan-400">check_circle</span> Live FPS & Server Ping Monitors</li>
          <li class="flex items-center gap-2 text-cyan-200"><span class="material-symbols-outlined text-sm text-cyan-400">check_circle</span> Custom CSV / JSON Log Exporters</li>
        </ul>
        <a href="https://wa.me/917047336254?text=Hi%20TPX%20STUDIO,%20I%20want%20an%20Analytics%20Dashboard." target="_blank" class="inline-flex items-center gap-1.5 font-display text-xs bg-cyan-400 text-black px-5 py-2.5 rounded-full font-bold transition-all">BUILD ANALYTICS UI <span class="material-symbols-outlined text-sm">arrow_forward</span></a>
      </div>
      <div class="glass-card p-5 rounded-2xl border-cyan-400/30 text-xs font-mono">
        <div class="text-cyan-400 font-bold mb-2">// Active Metrics Socket</div>
        <div class="space-y-2 text-gray-300">
          <div class="flex justify-between border-b border-white/10 pb-1"><span>Throughput</span><span class="text-green-400">12.4 GB/s</span></div>
          <div class="flex justify-between border-b border-white/10 pb-1"><span>Packet Drop</span><span class="text-green-400">0.00%</span></div>
          <div class="flex justify-between"><span>DDoS Mitigation</span><span class="text-cyan-400">Shielded</span></div>
        </div>
      </div>
    </div>`,
  web3: `
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
      <div>
        <div class="inline-block px-2.5 py-0.5 rounded-full bg-purple-400/10 border border-purple-400/30 text-purple-400 font-mono text-[10px] mb-2 font-bold">DECENTRALIZED UI</div>
        <h3 class="font-display text-xl sm:text-2xl font-bold text-white mb-2">Web3 & Crypto Gateway Portals</h3>
        <p class="mb-4 text-gray-300 text-xs sm:text-sm leading-relaxed">Secure dApp frontends integrated with MetaMask, Phantom, WalletConnect, smart contract read/write state managers, and gas-optimized checkout pipelines.</p>
        <ul class="space-y-1.5 font-mono text-xs text-text-dim mb-4">
          <li class="flex items-center gap-2 text-purple-200"><span class="material-symbols-outlined text-sm text-purple-400">check_circle</span> Multi-Chain Wallet Connectors</li>
          <li class="flex items-center gap-2 text-purple-200"><span class="material-symbols-outlined text-sm text-purple-400">check_circle</span> Token Gate Discord Sync</li>
        </ul>
        <a href="https://wa.me/917047336254?text=Hi%20TPX%20STUDIO,%20I%20want%20a%20Web3%20dApp." target="_blank" class="inline-flex items-center gap-1.5 font-display text-xs bg-purple-400 text-black px-5 py-2.5 rounded-full font-bold transition-all">LAUNCH WEB3 PORTAL <span class="material-symbols-outlined text-sm">arrow_forward</span></a>
      </div>
      <div class="glass-card p-5 rounded-2xl border-purple-400/30 text-xs font-mono">
        <div class="text-purple-400 font-bold mb-2">// Web3 Protocol State</div>
        <div class="space-y-2 text-gray-300">
          <div class="flex justify-between border-b border-white/10 pb-1"><span>RPC Node Ping</span><span class="text-green-400">8ms Edge</span></div>
          <div class="flex justify-between border-b border-white/10 pb-1"><span>Smart Contract</span><span class="text-gold-primary">Audited</span></div>
          <div class="flex justify-between"><span>Gas Optimization</span><span class="text-purple-400">Layer 2 Ready</span></div>
        </div>
      </div>
    </div>`
};
...
const serviceModalData = {
  ...
  voxel: {
    ...
    desc: "Specialized 3D modeling and animation designed for Minecraft servers, Hytale-like sandboxes, and custom game engines. Every asset is optimized with clean geometry and low polycounts to ensure rock-solid 60+ FPS.",
    photos: [
...
function openServiceModal(serviceKey) {
  ...
  const photoCards = data.photos.map((src, i) => `
    <div class="h-28 sm:h-40 rounded-2xl overflow-hidden border border-white/10 bg-[#121215]">
  ...
  container.innerHTML = `
    <span class="inline-block px-3 py-1 rounded-full bg-gold-primary/20 text-gold-primary font-mono text-[10px] sm:text-xs font-bold mb-2 sm:mb-3">${data.badge}</span>
    ...
    <div class="mb-4 sm:mb-6">
      <div class="font-mono text-xs text-gold-light font-bold mb-2 sm:mb-3 flex items-center gap-1">
    ...
    <div class="mb-4 sm:mb-6 bg-obsidian-dark/90 p-3.5 sm:p-5 rounded-2xl border border-white/10">
      <div class="font-mono text-[11px] sm:text-xs text-gold-primary font-bold mb-2 sm:mb-3">// TECHNICAL SPECIFICATIONS & DELIVERABLES</div>
    ...
    <div class="flex flex-col sm:flex-row items-center gap-2.5 sm:gap-3">
      <a href="${data.ctaLink}" target="_blank" class="w-full sm:w-auto font-display text-xs bg-gradient-to-r from-gold-primary via-gold-metallic to-gold-amber text-obsidian-black px-5 sm:px-6 py-3 rounded-full font-extrabold hover:glow-gold transition-all text-center flex items-center justify-center gap-2 shadow-xl">
...
// ==========================================
// 9. DOM INITIALIZATION & BLUR REVEAL OBSERVER
// ==========================================
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Staggered Blur-To-Sharp Scroll Observer
  const observer = new IntersectionObserver((entries) => {
    entries.forEach((entry, index) => {
      if (entry.isIntersecting) {
        // Stagger slight animation delays for child cards
        setTimeout(() => {
