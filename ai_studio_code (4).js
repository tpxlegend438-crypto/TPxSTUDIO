...
// ==========================================
// 4. INTERACTIVE SERVICE DETAIL MODAL (MORE PHOTOS & SPECS)
// ==========================================
...
function openServiceModal(serviceKey) {
  ...
  const photoCards = data.photos.map((src, i) => `
    <div class="h-32 sm:h-40 rounded-2xl overflow-hidden border border-white/10">
  ...
  container.innerHTML = `
    <span class="inline-block px-3 py-1 rounded-full bg-gold-primary/20 text-gold-primary font-mono text-[10px] sm:text-xs font-bold mb-3">${data.badge}</span>
    <h3 class="font-display text-2xl sm:text-3xl font-extrabold text-white mb-3">${data.title}</h3>
    <p class="text-xs sm:text-sm text-gray-300 leading-relaxed mb-6">${data.desc}</p>
    
    <div class="mb-6">
      <div class="font-mono text-xs text-gold-light font-bold mb-3 flex items-center gap-1">
    ...
    <div class="mb-6 bg-obsidian-dark/90 p-4 sm:p-5 rounded-2xl border border-white/10">
      <div class="font-mono text-xs text-gold-primary font-bold mb-3">// TECHNICAL SPECIFICATIONS & DELIVERABLES</div>
      <ul class="space-y-2">
        ${featureItems}
      </ul>
    </div>
    ...
    <div class="flex flex-col sm:flex-row items-center gap-3">
      <a href="${data.ctaLink}" target="_blank" class="w-full sm:w-auto font-display text-xs bg-gradient-to-r from-gold-primary via-gold-metallic to-gold-amber text-obsidian-black px-6 py-3.5 rounded-full font-extrabold hover:glow-gold transition-all text-center flex items-center justify-center gap-2 shadow-xl">
        <span class="material-symbols-outlined text-base">rocket_launch</span> ${data.ctaText}
      </a>
      <button onclick="closeServiceModal()" class="w-full sm:w-auto font-display text-xs bg-white/10 text-white px-5 py-3.5 rounded-full font-bold hover:bg-white/20 transition-all text-center">
        CLOSE WINDOW
      </button>
    </div>
  `;
